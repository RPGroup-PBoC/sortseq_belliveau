...
datadir = '../mass_spec/*/'
files = glob.glob(datadir+'*_relB*.csv')
...
df_summary_TF['bottom'] = np.exp(df_summary_TF['maxquant_ratio_normalized_avg_log'] -
                          df_summary_TF['maxquant_ratio_normalized_std_log'])
...
# Note that standard deviation is only based on only two measuremd ratios.
# We're not going to bother showing the error bars
...
plt.savefig(output+'fig4_relB_massspec_scatter.pdf', format='pdf')
