...
fname_1 = '20150513_marR_MG1655deltamarR_LB_na_mut1_4bins_summary.csv'
df1 = pd.read_csv(data_dir_1 + fname_1)
# df1['position'] = df1['position']# + 50.0

data_dir_2 = '../sortseq/20150513_marRmut1only_marRdeltaRAB_marRdeltaR/'
fname_2 = '20150513_marR_MG1655deltamarR_LB_na_mut2_4bins_summary.csv'
...
# use following to make relative to start codon of marR gene
# ind = np.arange(df.position.min(), df.position.max()+1)


# delta bin
ax1.bar(ind, df.groupby(['position'])['expshift'].mean(), width=0.8, \
        linewidth = 0, color = 'k')
# plot such that size matches length of lacZ promoter so that bar sizes/figures
# are similar.
...
figname_out = 'fig3_marR_deltaR_expshift.pdf'
