...
df = pd.read_csv('../../data/mass_spec/20151020_relB/proteinGroups_relB_reverse.txt',
                 delimiter='	')
...
df['std_dev'] = np.exp((df['Ratio H/L variability [%] relB_reverse'] / 100))

# from MaxQuant output, I want to collect the following for each replicate:
df = df[['Protein IDs','Gene names','Peptides', 'Intensity relB_reverse',
         'Intensity L relB_reverse', 'Intensity H relB_reverse',
         'Ratio H/L type', 'Ratio H/L relB_reverse', 'std_dev']]
