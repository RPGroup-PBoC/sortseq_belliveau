...
promoter = 'relB'
date = 20151020
WT_sequence = 'CAAAACTGCAG TAGTGCGGTAC TTGTAATGACATTTGTAATTACAA  GAGGTGTAAGACAT'
control_sequence = 'CAAAACTGCAG TAGTGCGGTAC TTAATGTGCAATTATGAATAGGAA  GAGGTGTAAGACAT'
...
df = pd.read_csv('../../data/mass_spec/20151020_relB/proteinGroups_relB_forward.txt',
                 delimiter='	')
# remove non-e.coli items detected
df = df[~df['Protein IDs'].str.contains("REV")]
df = df[~df['Protein IDs'].str.contains("CON")]

# Ratio H/L variability [%] = standard deviation of the natural logs
# of the constituent peptide ratios multiplied by 100.
df['std_dev'] = np.exp((df['Ratio H/L variability [%] relB_forward'] / 100))

# from MaxQuant output, I want to collect the following for each replicate:
df = df[['Protein IDs','Gene names','Peptides', 'Intensity relB_forward',
         'Intensity L relB_forward', 'Intensity H relB_forward',
         'Ratio H/L type', 'Ratio H/L relB_forward', 'std_dev']]
