...
csv_bool_dates = np.array([('csv' in f or 'txt' in f)\
                    for f in files])
files_dates = files[np.array(csv_bool_dates)]

df_dates = pd.DataFrame()
for f in files_dates:
    df_dates = df_dates.append([f[:8]],ignore_index=True)

for date in df_dates[0].unique():
